...
class KongSlotMachine {
    constructor(config) {
        ...
        // Sound - use the existing AudioManager if available or create a new one
        this.audioManager = window.audioManager || this.createAudioManager();
        
        // Initialize
        this.initPixiApp();
    }
    
    // Create a new AudioManager instance if needed
    createAudioManager() {
        const manager = {
            sounds: {},
            ...
            createSound: function(src, loop = false, volume = 1.0) {
        ...
        manager.sounds.background = manager.createSound('sounds/SE_Base_BG.mp3', true, 0.5);
        manager.sounds.bigWin = manager.createSound('sounds/SE_BigWin.mp3', false, 0.8);
        manager.sounds.reelSpeedUp = manager.createSound('sounds/SE_ReelSpeedUp.mp3', false, 0.9);
        manager.sounds.freeSpin = manager.createSound('sounds/SE_Free_BG.mp3', true, 0.9);
        manager.sounds.spin = manager.createSound('sounds/SE_ButtonPress.mp3', false, 1.0);
        manager.sounds.spinStop = manager.createSound('sounds/SE_Spinstop.mp3', false, 0.9);
        manager.sounds.win = manager.createSound('sounds/SE_Scoring_01.mp3', false, 1.0);
        manager.sounds.jackpot = manager.createSound('sounds/SE_Jackpot.mp3', false, 1.0);
        manager.sounds.megaWin = manager.createSound('sounds/SE_MegaWin.mp3', false, 1.0);
        manager.sounds.ultraWin = manager.createSound('sounds/SE_UltraWin.mp3', false, 1.0);
    ...
    showBigWin(amount) {
        ...
        if (!winOverlay) {
            console.warn('Win overlay element not found');
            return;
        }
    ...
    try {
        ...
        if (hideKongOverlayBtn) {
            hideKongOverlayBtn.addEventListener('click', () => {
                ...
                if (window.slotMachine && window.slotMachine.audioManager) {
